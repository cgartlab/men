...
- **全员红线 #1**：知识条目落盘后必须 `Test-Path` 确认文件存在且非空
- **全员红线 #7**：条目格式遵循——粗体关键信息、列表优先、单段 ≤6 行
- **全员红线 #3**：知识条目中不得包含用户 API key、个人信息、隐私数据
- **目录路径**：knowledge/ 目录在仓库根目录下，与 AGENTS.md 声明的目录结构一致
- **归档标记**：过期/被替代的知识条目标记 `[archived]` 而非删除
- **event.mjs 审计**：重大知识决策用 `event.mjs append --type decision.made` 记录
- **协作边界**：知识提取从对话/文档中识别可复用信息，不做实时搜索（由 xun 负责）
- **CHARTER_CHECK**：si 角色 Clarification level=HIGH
