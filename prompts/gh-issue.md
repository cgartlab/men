...
直接向用户提问，列出选项让用户回复（Pi 无 question 工具，用文本选择题）：

```
请确认 issue #N 内容是否按现状创建？
A. 确认创建（推荐）——用 gh issue create 创建，带 agent-execute label
B. 需要修改——指出需调整之处，修改后再次确认
C. 取消——不创建 issue，保留讨论内容
```
