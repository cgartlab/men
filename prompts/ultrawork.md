...
**任务清单**：创建完整任务清单（10 步 + 预估子任务），标记第 1 步 `in_progress`。
...
**任务清单**：标记第 2 步 `completed`，标记第 3 步 `in_progress`。

- team 类和 hyperplan 类任务：**必须先 spawn si**（`subagent` 工具，agent: "si"），要求产出 `<plan>` envelope
- si 规划耗时较长，可与其他子任务并行发起，men 继续响应用户
- si 返回后，将 plan 中的子任务添加到任务清单中
- si 的 plan 必须包含：
  - **任务依赖图**：各子任务间的先后依赖关系
...
**任务清单**：标记第 3 步 `completed`，标记第 4 步 `in_progress`。每个子任务在任务清单中有对应项。

- 严格按 plan 中的并行波次执行，**不能打乱顺序**（Wave 2 依赖 Wave 1 的产物）
- **并行上限 ≤ 4 个同时**（避免资源争用与输出混乱）
- **并行 spawn 模式**：Wave 内多个无依赖子任务，在**单条消息中发起多个 subagent 调用**：
  ```
  // 单消息并行（正确）
  subagent({ agent: "xun", task: "查数据：<完整自洽 prompt>" })
  subagent({ agent: "yi", task: "设计配图：<完整自洽 prompt>" })
  ```
- **长耗时子任务**（xun 搜索、chi judge、si 规划、yi 生图）与其他子任务并行发起，men 继续响应用户
...
**任务清单**：每个子 agent 返回后，立即标记对应子任务 `completed`（或失败时标记 `pending` 并备注原因）。
...
- 把 ji / yi 等产出角色的结果**回传给 si 评估**（用 subagent 工具再 spawn 一次 si，传入产物摘要和原验收标准）
...
- 机械门禁通过后（或产物不涉及工程时），**必须 spawn chi**（`subagent` 工具，agent: "chi"）做 fresh-context judge
...
**任务清单**：标记第 4-7 步 `completed`，标记第 8 步 `in_progress`。确认清单与最终汇报一致。
...
**任务清单**：标记第 8 步 `completed`，标记第 10 步 `in_progress`。
...
LEARN 完成后，标记第 10 步 `completed`，确认任务清单全部完成。
