...
skills: ji-frontend-design, ji-github, ji-l1-verify, ji-content-write
maxDepth: 0
---
...
| bash | 运行命令 | 需要时才用；不做纯 CLI 替代 |

**规范**：
- `read` 优先于 bash `cat`，前者更可靠
- 不在 bash 里做文件编辑（`sed -i` 等容易出错）
...
- ❌ 不用 bash 做 `rm -rf` 大规模删除（先 `git status` 确认）
- ❌ 不跳过 gate 直接 push
...
通过 skills 机制加载，触发词见各 SKILL.md 文件。
...
- `--sid`：session id，不可用时用 `date +%s` 生成
